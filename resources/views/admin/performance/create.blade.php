...
@section('title', 'Thêm đánh giá KPI')
                    ...
                    <form action="{{ route('performance.store') }}" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-2 mb-3">
                                ...
                                <input type="number" name="overall_score" id="overall_score" class="form-control @error('overall_score') is-invalid @enderror" value="{{ old('overall_score') }}" min="0" max="100" required>
                                @error('overall_score')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="strengths" id="strengths" class="form-control @error('strengths') is-invalid @enderror" rows="2">{{ old('strengths') }}</textarea>
                            @error('strengths')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            <textarea name="weaknesses" id="weaknesses" class="form-control @error('weaknesses') is-invalid @enderror" rows="2">{{ old('weaknesses') }}</textarea>
                            @error('weaknesses')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            <textarea name="improvements" id="improvements" class="form-control @error('improvements') is-invalid @enderror" rows="2">{{ old('improvements') }}</textarea>
                            @error('improvements')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <table class="table table-bordered align-middle">
                                ...
                                <tbody>
                                    @foreach($kpis as $kpi)
                                        <tr>
                                            <td>{{ $kpi->name }}</td>
                                            <td><input type="number" step="0.01" name="kpi[{{ $kpi->id }}][target_value]" class="form-control" value="{{ old('kpi.'.$kpi->id.'.target_value') }}"></td>
                                            <td><input type="number" step="0.01" name="kpi[{{ $kpi->id }}][actual_value]" class="form-control" value="{{ old('kpi.'.$kpi->id.'.actual_value') }}"></td>
                                            <td><input type="number" step="0.01" name="kpi[{{ $kpi->id }}][score]" class="form-control" value="{{ old('kpi.'.$kpi->id.'.score') }}"></td>
                                            <td><input type="text" name="kpi[{{ $kpi->id }}][comment]" class="form-control" value="{{ old('kpi.'.$kpi->id.'.comment') }}"></td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
